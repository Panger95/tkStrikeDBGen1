...
run:
	cd "Import Thresholds Gen 1" && $(JAVA_CMD) -cp ReplaceDB.jar ReplaceDB.Main
	sleep 25
	@if [ "$(OS)" == "Darwin" ]; then \
		cd "Import Thresholds Gen 1" && $(JAVA_CMD) -cp Mac.jar ReplaceDB.Mac; \
	elif [ "$(OS)" == "Windows_NT" ]; then \
		cd "Import Thresholds Gen 1" && $(JAVA_CMD) -cp Windows.jar ReplaceDB.Windows; \
	fi
